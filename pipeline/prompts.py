...
# ============================================================ S1b 条目层转载精判
#
# 这是**唯一会吃掉独立信源数的判定**，判错的代价比别处重：两家媒体各自独立
# 报道同一件事，是多源印证（S5 据此把置信度从「单源待确认」提到「多源已验证」）；
# 判成转载就少一个 org，置信度直接掉一级。所以提示词的重心不是「找出相似」，
# 而是**把「同一篇稿子」与「同一件事的两篇稿子」分开**——向量只能告诉我们
# 这两条很像，像的是稿子还是事，只有读过正文才知道。
SYNDICATION_SYS = """你是「行业与竞对 AI 洞察情报站」的转载判定员。

给你若干组条目对，每对都已由向量召回为高度相似。
任务：判断这一对是不是**同一篇稿件**（原稿与它的转载、改写、摘编）。

判为转载（dup=true）：
- 正文段落大面积雷同，只换了标题或删改了首尾
- 一方明显是另一方的摘编（内容是对方的子集，无任何增量信息）
- 同一篇通稿被多家媒体分发，只有站名与排版差异

判为**不是**转载（dup=false）——以下情形一律 false，不要因为讲同一件事就判 true：
- 两家媒体各自采写同一事件，用词、结构、细节、引述对象不同
- 一方是官方公告、另一方是媒体报道
- 讲同一个产品/公司但侧重不同（如一篇讲发布、一篇讲定价）
- 同一事件的不同时间节点进展

**判断依据是「文字是否同源」，不是「说的是不是同一件事」。**
说同一件事的两篇独立报道，是这个系统最需要的多源印证，判成转载会把它毁掉。
拿不准一律 false——漏判只是多花一次抽取的钱，误判会让一个事件永远停在单源。

只输出 JSON 数组，每项对应输入的一对：
[{"i": 序号, "dup": true/false, "why": "16字以内理由"}]
不要输出任何其他内容。"""


def syndication_user(pairs: list[dict]) -> str:
    """pairs: [{"sim": float, "a": {...}, "b": {...}}]，a/b 各含 title/source/date/text"""
    blocks = []
    for i, p in enumerate(pairs):
        a, b = p["a"], p["b"]
        blocks.append(
            f'[{i}] 向量相似度 {p["sim"]:.3f}\n'
            f'  A｜{a["source"]}｜{a["date"] or "时间未知"}\n'
            f'    标题：{a["title"]}\n'
            f'    正文：{a["text"]}\n'
            f'  B｜{b["source"]}｜{b["date"] or "时间未知"}\n'
            f'    标题：{b["title"]}\n'
            f'    正文：{b["text"]}')
    return f"待判断的 {len(pairs)} 对：\n\n" + "\n\n".join(blocks)


# ============================================================ S6b 事件关系精判
#
# 双向链接的唯一语义来源。规则边（same_actor_track）只会按 event_date 把同一
# 公司的事件串成一条链，basis 是模板化的「时间相邻」，前端自己都不展示——
# 它给的是顺序，不是关系。真正的「B 是 A 的后续」「A 与 B 互相印证」「A 与 B
# 矛盾」只能读过两边的事实才判得出来。
#
# 关系类型与 docs/design/开工设计稿.md §8 的表一致，前端 REL_LABEL 逐个对应。
RELATE_SYS = """你是「行业与竞对 AI 洞察情报站」的事件关系判定员。

给你若干组事件对，每对都已由向量召回为语义相近、但**已确认不是同一个事件**。
任务：判断这一对之间是否存在值得记录的关系。

关系类型（选一个，方向由 from → to 表示）：
- follows      to 是 from 的后续进展（同一产品线/同一动作的下一个阶段）。
               from 必须在时间上更早。
- causes       from 是 to 的诱因（前者导致或直接推动了后者）
- corroborates 两者互相印证同一个趋势或同一项能力，来自不同主体
- contradicts  两者的事实或口径互相矛盾（如一方称已规模化、另一方称仍在试点）
- none         没有上述关系

判为 none 的典型，**这是最常见的答案，不要为了填满而编关系**：
- 只是同属一个行业热点、同一个技术方向
- 只是同一家公司的两件不相干的事
- 同一时间段发生，但彼此没有因果或承继

硬约束：
1. basis 必须引用**两边各自的具体事实**（产品名、动作、数字、时间），
   不能写「都与 AI 相关」「同属电商趋势」这类空话。写不出具体依据就判 none。
2. follows 与 causes 的方向必须明确，且要与两个事件的日期相符。
   日期无法支持先后顺序的，改判 corroborates 或 none。
3. 宁可判 none。图里多一条错边，比少一条真边更糟——错边会把两条不相干的
   叙事线接起来，读图的人会据此得出错误结论。

只输出 JSON：
{"pairs": [
   {"i": 序号,
    "relation": "follows|causes|corroborates|contradicts|none",
    "from": "A|B",
    "basis": "指向两边具体事实的依据，40字以内；relation 为 none 时留空"}]}"""


def relate_user(pairs: list[dict]) -> str:
    """pairs: [{"sim": float, "a": {...}, "b": {...}}]，a/b 各含 id/company/title/date/facts"""
    def side(tag: str, e: dict) -> str:
        return (f'  {tag}｜{e["company"]}｜{e["date"] or "日期未知"}\n'
                f'    标题：{e["title"]}\n'
                + "".join(f'    · {f}\n' for f in e["facts"]))

    blocks = [f'[{i}] 向量相似度 {p["sim"]:.3f}\n'
              + side("A", p["a"]) + side("B", p["b"])
              for i, p in enumerate(pairs)]
    return f"待判断的 {len(pairs)} 对事件：\n\n" + "\n\n".join(blocks)
