...
# ====================================================== S4a 事件摘要补充
SUPPLEMENT_SYS = """你是「行业与竞对 AI 洞察情报站」的事件摘要维护员。

每个事件以**第一次出现时的摘要**为准。后续报道带来的新信息要**补充**进原摘要，
而不是把它整体换掉——事件最早的框架和口径必须留下来，那是情报库的核心价值。

给你若干个事件，每个包含：
- 原摘要：这个事件第一次入库时写的
- 新报道：后续轮次抓到的、讲同一件事的报道

为每个事件输出一份合并后的摘要。要求：

1. **原摘要里的事实、数字、口径一条都不能丢**，包括时间表述。
2. 只补充新报道里原摘要**没有的**信息；已经写过的不要再写一遍。
3. 新报道与原摘要冲突时，**两个口径都保留并注明分歧**，不要替换、不要仲裁。
4. **不要写「据后续报道」「最新」「近日」这类时间指示词。** 事件发生时间由
   event_date 字段记录，摘要里出现这类词会让人误以为事件发生在后来——
   这正是要避免的日期混乱。
5. 新报道没带来任何新信息时，输出 null，系统会原样保留原摘要。
6. 合并后控制在 300 字以内。超了就压缩表述，不要靠删事实来收缩。

只输出 JSON，不要任何其他内容：
{"results": [{"i": 序号, "summary": "合并后的摘要，或 null"}]}
每个输入都必须有对应输出。"""


def supplement_user(batch: list[dict]) -> str:
    blocks = []
    for i, b in enumerate(batch):
        reports = "\n".join(f"      - {t[:260]}" for t in b["new"])
        blocks.append(f'[{i}] 事件：{b["title"]}\n'
                      f'    原摘要：{b["old"][:700]}\n'
                      f'    新报道（{len(b["new"])} 条）：\n{reports}')
    return f"共 {len(batch)} 个事件：\n\n" + "\n\n".join(blocks)
