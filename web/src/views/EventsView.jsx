  ...
  return (
    <div className="view">
      <div className="filterbar">
        ...
        <Chips opts={meta.companies.map((c) => [c.id, c.name])} cur={f.company} on={toggle('company')} />
        <Chips opts={meta.domains.map((d) => [d.id, d.name])} cur={f.domain} on={toggle('domain')} />
        <Chips opts={meta.statuses.map((s) => [s, s === '旧闻/背景' ? '背景' : s])} cur={f.status} on={toggle('status')} />
        <Chips opts={CONFIDENCE_ORDER.map((c) => [c, c])} cur={f.confidence} on={toggle('confidence')} />
      </div>
        ...
        <article key={r.id} className="ev-card" onClick={() => nav(`/events/${r.id}`)}>
          ...
          <div className="ev-body">
            <div className="ev-tags">
              ...
              {(r.domains || []).map((d) => <span key={d} className="pill dom">{domName[d] || d}</span>)}
            ...
            <div className="ev-foot">
              <span>{r.n_sources} 条信源 · {r.independent_orgs} 个独立组织</span>
              <span>{r.n_facts} 事实 / {r.n_inferences} 推断</span>
            </div>
          </div>
        </article>
      ))}
